...
class KilledMonsterSignalsTestCase(TestCase):
    ...
    def test_register_if_user_win_trophy_killed_monster(self):
        category = Category.objects.create(description='numbers killed monsters')
        ...
        killed_monster_godzilla = KilledMonster.objects.create(user=self.user, monster=monster_godzilla)
        killed_monster_kingkong = KilledMonster.objects.create(user=self.user, monster=monster_kingkong)
    ...
    def test_register_if_user_win_trophy_killed_monster_others_levels(self):
        ...
        trophies_godzilla = trophies.filter(value_register_field=monster_godzilla.name)
        trophies_kingkong = trophies.filter(value_register_field=monster_kingkong.name)
